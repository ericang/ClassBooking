#!"C:/Program Files (x86)/Ampps/php-5.6/php-cgi.exe"
<?php
...
// Is the license active ? If not just show him the License Info !
if(asperapp(0 ,1 ,1) && (empty($globals['license']) || $globals['license'] == '00000-00000-00000-00000-00000')){
	...
	$globals['stop_buffer_process'] = false;
	
	if(!in_array(optGET('act'), array('login', 'license','licensekey', 'logout'))){
		redirect('act=licensekey');
		die();		
	}
}
...
if(webuzo()){
	...
	if($url_ses[1] != $SESS['token']  && !in_array(optGET('act'), array('login', 'logout', 'license')) && empty($softpanel->isapi)) {
		redirect('act=logout');	
		die();
	}
	
	if($softpanel->is_sysapps_disable() && !in_array(optGET('act'), array('login', 'logout', 'license')) && empty($softpanel->isapi)){
		redirect('act=logout');	
		die();
	}
...
switch($act){
	...
	// Add User
	case 'adduser':
	include_once($globals['adminfiles'].'/adduser.php');
	adduser();
	break;	
	
	// List User
	case 'listuser':
	include_once($globals['adminfiles'].'/listuser.php');
	listuser();
	break;
	
	// List Domains
	case 'listdomains':
	include_once($globals['adminfiles'].'/listdomains.php');
	listdomains();
	break;
	
	// List Domains
	case 'adddomain':
	include_once($globals['adminfiles'].'/adddomain.php');
	adddomain();
	break;
	
	// Disable Auto Upgrade
	case 'no_autoupgrade':
	include_once($globals['adminfiles'].'/no_autoupgrade.php');
	no_autoupgrade();
	break;
	
	// Advanced Software Settings
	case 'adv_software':
	include_once($globals['adminfiles'].'/adv_software.php');
	adv_software();
	break;
	
	// Import / Export Settings
	case 'import_export':
	include_once($globals['adminfiles'].'/import_export.php');
	import_export();
	break;
	
	// Scripts Requirements
	case 'script_requirement':
	include_once($globals['adminfiles'].'/script_requirement.php');
	script_requirement();
	break;
	
	// Synchronize Settings
	case 'sync_settings':
	include_once($globals['adminfiles'].'/sync_settings.php');
	sync_settings();
	break;
	
	// Change root password
	case 'root_pass':
	include_once($globals['adminfiles'] . '/root_pass.php');
	root_pass();
	break;
	
	// Enable / Disable Options !!!
	case 'webuzo_acl':
	include_once($globals['adminfiles'] . '/webuzo_acl.php');
	webuzo_acl();
	break;
	
	// Statistics
	case 'ins_statistics':
	include_once($globals['adminfiles'] . '/ins_statistics.php');
	ins_statistics();
	break;
}
